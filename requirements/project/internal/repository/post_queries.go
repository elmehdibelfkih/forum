...
const (
	...
	// INSERT_NEW_DISLIKE = `INSERT INTO likes_dislikes (user_id, post_id, is_like, is_dislike) VALUES (?, ?, ?, ?)`
	...
	// UDDATE queries
	// UPDATE_LIKE    = `UPDATE likes_dislikes WHERE user_id = ? AND post_id = ? SET is_like = ? AND is_dislike = 0`
	// UPDATE_DISLIKE = `UPDATE likes_dislikes WHERE user_id = ? AND post_id = ? SET is_like = 0 AND is_dislike = ?`
